...
public class MemcachedLoginSessionManager extends AbstractLoginSessionManager {
	...
	public UserContext getUserContextAndUpdateExpire(HttpServletRequest request) {
		...
		return newUserContext(request, uid, sessionId, tpId, admin);
	}
	...
	public void login(HttpServletRequest request, HttpServletResponse response,
		...
		if (persistent) {
			savePersistLoginToken(request, response, uid);
		}
		// request里保存userContext
		request.setAttribute("context",
				newUserContext(request, uid, token, tpId, isAdmin));
	}
	...
	public void updateTpId(HttpServletRequest request,
		...
		if (StringUtils.isNotEmpty(token)) {
			...
			request.setAttribute("context",
					newUserContext(request, uid, token, tpId, false));
		}
	}
}
