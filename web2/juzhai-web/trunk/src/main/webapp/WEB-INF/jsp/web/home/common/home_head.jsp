	...
	<div class="m">
		<div class="my_head_area"><!--my_head_area begin-->
			<c:choose>
				...
				<c:otherwise><div class="face"><a href="/profile/index/face"><img src="${jzr:userLogo(profile.uid,profile.newLogoPic,180)}" width="180" height="180" /></a><c:choose><c:when test="${profile.logoVerifyState == 1||profile.logoVerifyState == -1}"><b>审核中...</b></c:when><c:when test="${profile.logoVerifyState == 3}"><b>未通过审核</b></c:when></c:choose></div></c:otherwise>
			</c:choose>
