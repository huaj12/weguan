...
<div class="title"><!--title begin-->
	<h2 class="<c:choose><c:when test='${loginUser.gender==1}'>boy</c:when><c:otherwise>girl</c:otherwise></c:choose>">
		<span><c:choose><c:when test="${loginUser!=null}"><c:out value="${loginUser.nickname}" /></c:when><c:otherwise>小宅</c:otherwise></c:choose></span>,周末想找个伴儿么？
	</h2>
	...
	<div class="title_right"><!--title_right begin-->
		<a href="/showUsers" <c:if test="${userType=='showUsers'}">class="active"</c:if>>有缘人</a>
		<a href="/showFollows" <c:if test="${userType=='showFollows'}">class="active"</c:if>>认识的</a>
	</div><!--title_right end-->
</div><!--title end-->