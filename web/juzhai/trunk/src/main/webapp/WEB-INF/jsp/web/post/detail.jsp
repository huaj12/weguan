							...
							<div class="m">
								<div class="person <c:choose><c:when test="${postProfile.gender == 1}">boy</c:when><c:otherwise>girl</c:otherwise></c:choose>"><!--person begin-->
									...
									<c:if test="${context.uid != postProfile.uid}">
										...
										<div class="mail"><a class="send-message" href="javascript:void(0);" title="发私信给ta" target-uid="${postProfile.uid}" target-nickname="<c:out value="${postProfile.nickname}"/>">私信</a></div>
									</c:if>
								</div><!--person end-->
							</div>
							<div class="t"></div>
