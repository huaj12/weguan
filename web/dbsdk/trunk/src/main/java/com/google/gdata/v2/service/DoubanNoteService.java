...
import com.google.gdata.v2.model.note.DoubanNoteEntryObj;
import com.google.gdata.v2.model.note.DoubanNoteFeedObj;
...
public class DoubanNoteService extends DoubanService {
	...
	final static Logger logger = Logger.getLogger(DoubanNoteService.class
			.getName());
	...
	public DoubanNoteEntryObj getNoteById(long noteId) throws DoubanException,
			IOException {
		String url = RequestUrls.DOUBAN_NOTE_PREFIX + "/" + noteId;
		DoubanNoteEntryObj result = this.client.getResponse(url, null,
				DoubanNoteEntryObj.class, false);
		return result;
	}

	public DoubanNoteFeedObj getAllNotesFromUser(String userId)
	...
	public DoubanNoteFeedObj getAllNotesFromUser(String userId,
		...
		DoubanNoteFeedObj result = this.client.getResponse(url, params,
				DoubanNoteFeedObj.class, false);
		return result;
	}

	public boolean createNewNote(String title, String content,
			boolean isPrivate, boolean canReply, String accessToken)
			throws DoubanException, IOException {
		setAccessToken(accessToken);
		DoubanNoteEntryObj entry = generateDoubanNoteEntry(title, content,
				isPrivate, canReply);
		if (entry == null) {
			throw ErrorHandler.getCustomDoubanException(100,
					"Note data is not correct, please double check");
		}
		try {
			int responseCode = this.client.postResponseCodeOnly(
					RequestUrls.DOUBAN_NOTE_PREFIX + "s", entry, true);
	...
	public boolean updateNote(long noteId, String title, String content,
		...
		try {
			int responseCode = this.client.putResponseCodeOnly(
					RequestUrls.DOUBAN_NOTE_PREFIX + "/" + noteId, entry, true);
	...
	public boolean deleteNote(long noteId, String accessToken)
			throws DoubanException, IOException {
		setAccessToken(accessToken);
		try {
			int responseCode = this.client.deleteResponse(
					RequestUrls.DOUBAN_NOTE_PREFIX + "/" + noteId, true);
	...
	private DoubanNoteEntryObj generateDoubanNoteEntry(String title,
			String content, boolean isPrivate, boolean canReply) {
