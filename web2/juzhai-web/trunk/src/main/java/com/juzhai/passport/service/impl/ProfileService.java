...
public class ProfileService implements IProfileService {
	...
	private boolean hasTown(long cityId) {
		boolean flag = false;
	...
	private void update(Profile profile) throws ProfileInputException {
		...
		if (hasTown(profile.getCity()) && profile.getTown() == -1) {
			throw new ProfileInputException(
	...
	@Override
	public void updateLogoAndProfile(long uid, Profile profile, String logoPath)
			throws UploadImageException, ProfileInputException {
		String fileName = null;
		if (StringUtils.isNotEmpty(logoPath)) {
			fileName = profileImageService.saveImg(uid, logoPath);
		}
		if (StringUtils.isNotEmpty(fileName)) {
			profile.setNewLogoPic(fileName);
			profile.setLogoVerifyState(LogoVerifyState.VERIFYING.getType());
		}
	...
	@Override
	public void guideLogoAndProfile(long uid, Profile profile, String logoPath)
			throws UploadImageException, ProfileInputException {
		String fileName = null;
		if (StringUtils.isNotEmpty(logoPath)) {
			fileName = profileImageService.saveImg(uid, logoPath);
