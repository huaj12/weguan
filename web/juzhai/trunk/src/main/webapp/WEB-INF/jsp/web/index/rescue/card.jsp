...
<c:choose>
	<c:when test="${postView != null}">
		...
		<div class="btn"><a href="javascript:void(0);" class="xy" post-id="${postView.post.id}" resp-count="${postView.post.responseCnt}" rescue-uid="${postView.profileCache.uid}">响应ta</a> <a href="javascript:void(0);" class="next" rescue-uid="${postView.profileCache.uid}">换一个</a></div>
	</c:when>
	<c:otherwise>
