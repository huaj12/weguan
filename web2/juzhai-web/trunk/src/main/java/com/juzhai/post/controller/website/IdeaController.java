...
public class IdeaController extends BaseController {
	...
	public String detail(HttpServletRequest request, Model model,
		...
		long cityId = 0L;
		if (context.hasLogin()) {
			ProfileCache loginUser = profileService
					.getProfileCacheByUid(context.getUid());
	...
	public String showIdeaWidget(HttpServletRequest request, Model model,
		...
		Long cityId = 0L;
		if (context.hasLogin()) {
			ProfileCache profile = profileService.getProfileCacheByUid(context
					.getUid());
			if (null != profile) {
				cityId = profile.getCity();
			}
		}
		List<Idea> ideaList = ideaService.listUnUsedWindowIdea(
				context.getUid(), cityId, page - 1, 1);
