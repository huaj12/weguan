...
import com.google.gdata.v2.model.common.DoubanEntityObj;
import com.google.gdata.v2.model.doumail.DoubanMailEntryObj;
import com.google.gdata.v2.model.doumail.DoubanMailFeedObj;
...
public class DoubanMailService extends DoubanService {
	...
	final static Logger logger = Logger.getLogger(DoubanMailService.class
			.getName());
	...
	public DoubanMailFeedObj getMailsFromInbox() throws DoubanException,
			IOException {
		return getMailsFromInbox(null, null);
	}

	public DoubanMailFeedObj getMailsFromInbox(Integer startIndex,
			Integer maxResult) throws DoubanException, IOException {
		...
		DoubanMailFeedObj result = this.client.getResponse(url, params,
				DoubanMailFeedObj.class, true);
		return result;
	}

	public DoubanMailFeedObj getMailsFromOutbox() throws DoubanException,
			IOException {
		return getMailsFromOutbox(null, null);
	}

	public DoubanMailFeedObj getMailsFromOutbox(Integer startIndex,
			Integer maxResult) throws DoubanException, IOException {
		List<NameValuePair> params = new ArrayList<NameValuePair>();
		if (startIndex != null) {
			params.add(new BasicNameValuePair("start-index", "" + startIndex));
	...
	public DoubanMailFeedObj getUnreadMails() throws DoubanException,
			IOException {
		return getUnreadMails(null, null);
	}

	public DoubanMailFeedObj getUnreadMails(Integer startIndex,
			Integer maxResult) throws DoubanException, IOException {
		List<NameValuePair> params = new ArrayList<NameValuePair>();
		if (startIndex != null) {
			params.add(new BasicNameValuePair("start-index", "" + startIndex));
	...
	public DoubanMailEntryObj getMailById(long mailId, boolean keepUnread)
		...
		DoubanMailEntryObj result = this.client.getResponse(url, params,
				DoubanMailEntryObj.class, true);
		return result;
	}
	 ...
	 * @throws IOException
	 */
	public boolean sendMail(String receiverId, String content, String title)
			throws DoubanException, IOException {
		String url = RequestUrls.DOUBAN_MAIL_PREFIX + "s";
		DoubanMailEntryObj entry = generateDoubanMailEntryObj(receiverId,
				content, title);
		if (entry == null) {
			throw ErrorHandler.getCustomDoubanException(100,
					"Illegal mail data provided");
		}
		try {
	...
	public boolean markMailAsRead(long mailId) throws IOException,
			DoubanException {
		String url = RequestUrls.DOUBAN_MAIL_PREFIX + "/" + mailId;
		DoubanMailEntryObj requestEntry = generateOnlyAttribute(null, true);
		try {
			int responseCode = this.client.putResponseCodeOnly(url,
					requestEntry, true);
			if (responseCode != StatusCode.HTTP_STATUS_CREATED) {
				return false;
			}
	...
	public boolean markMailReadInBatch(List<Long> ids) throws IOException,
			DoubanException {
	...
	public boolean deleteMailsInBatch(List<Long> ids) throws IOException,
			DoubanException {
		List<DoubanMailEntryObj> entries = new ArrayList<DoubanMailEntryObj>();
		for (long id : ids) {
		...
		try {
			int responseCode = this.client
					.postResponseCodeOnly(url, feed, true);
			if (responseCode != StatusCode.HTTP_STATUS_OK) {
				return false;
			}
			return true;
	...
	private DoubanMailEntryObj generateDoubanMailEntryObj(String receiverId,
			String content, String title) {
		if (content == null || content.isEmpty() || title == null
				|| title.isEmpty() || receiverId == null
				|| receiverId.isEmpty()) {
			return null;
		}
