	...
	<table>
		<tr>
			...
			<c:forEach var="c" items="${citys}">
				<option <c:if test="${city==c.id}"> selected="selected"</c:if> value="${c.id }">${c.name}</option>
			</c:forEach>	
			</select></td>
		</tr>
		<tr>
		...
		<tr>	
			...
			<td><input type="text" name="dateString" readonly="readonly" onclick="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd HH:mm:ss'});" value="<fmt:formatDate value="${date}"
