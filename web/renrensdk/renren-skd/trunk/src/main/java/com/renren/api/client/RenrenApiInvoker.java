...
public final class RenrenApiInvoker {
	...
	private static final long serialVersionUID = -8722424046861659208L;

	private RenrenApiConfig rrApiConfig = null;
	...
	public RenrenApiInvoker(String token, boolean isAccessToken,
			RenrenApiConfig apiConfig) {
		this.token = token;
		this.isAccessToken = isAccessToken;
		this.rrApiConfig = apiConfig;
	}
	...
	public String uploadPhoto(TreeMap<String, String> parameters,
			String filename, String contentType, byte[] data) {
		prepareParams(parameters, FORMAT_JSON);
		String content = HttpURLUtils.doUploadFile(
				RenrenApiConfig.renrenApiUrl, parameters, "upload", filename,
				contentType, data);
		if (content.indexOf("error_code") >= 0) {
			throw this.parseRenrenApiException(content);
		}
	...
	private TreeMap<String, String> prepareParams(
			TreeMap<String, String> params, String format) {
		params.put("v", RenrenApiConfig.renrenApiVersion);
		params.put("call_id", String.valueOf(System.currentTimeMillis()));
		params.put("format", format);
		params.put("api_key", rrApiConfig.renrenApiKey);
	...
	private TreeMap<String, String> sigParams(TreeMap<String, String> params) {
		...
		sb.append(rrApiConfig.renrenApiSecret);
	...
	private RenrenApiException parseRenrenApiException(String errorJson) {
		...
		RenrenApiException exception = new RenrenApiException(
				Integer.parseInt(errorCode), errorMsg);
		return exception;
	}
}
