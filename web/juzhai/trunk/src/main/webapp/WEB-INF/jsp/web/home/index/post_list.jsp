					...
					<div class="wtg"><!--wtg begin-->
						...
						<div class="btn"><!--btn begin-->
							...
							<c:if test="${not empty isQplus && isQplus}">	
									<div class="share_icon"><a href="javascript:void(0);" onclick="qPlusShare('我想找伴去:${postView.post.content}<c:if test='${postView.post.date != null}'>时间:<fmt:formatDate value='${postView.post.date}' pattern='yyyy.MM.dd'/></c:if> <c:if test='${not empty postView.post.place}'>地点:${postView.post.place}</c:if>','','${jzr:postPic(postView.post.id, postView.post.ideaId, postView.post.pic, 200)}','','拒宅网');return false;" title="分享">分享</a></div>
							</c:if>
