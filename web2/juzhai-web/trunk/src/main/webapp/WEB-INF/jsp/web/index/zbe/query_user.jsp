													...
													<div class="pub_box mouseHover <c:choose><c:when test='${view.profile.gender==1}'>boy</c:when><c:otherwise>girl</c:otherwise></c:choose>"><!--pub_box begin-->
														...
														<div class="pub_box_m"><!--pub_box_m begin-->
															...
															<c:if test="${context.uid != view.profile.uid}">
																...
																<div class="keep user-remove-interest remove-interest-${view.profile.uid}" <c:if test="${!view.hasInterest}">style="display: none;"</c:if>><a href="javascript:void(0);" class="done" uid="${view.profile.uid}" title="点击取消关注">已关注</a></div>
																<div class="keep user-add-interest interest-${view.profile.uid}" <c:if test="${view.hasInterest}">style="display: none;"</c:if>><a href="javascript:void(0);" uid="${view.profile.uid}" title="点击关注">关注ta</a></div>
