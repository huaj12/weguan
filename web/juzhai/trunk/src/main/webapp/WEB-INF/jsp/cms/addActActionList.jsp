	...
	<table border="0" cellspacing="4">
		...
		<c:forEach var="list" items="${lists}">
			<tr>
				...
				<td><a onclick="deleteAddAct('${list.id}')" href="/cms/showCreateAct?actName=${list.name}&addUid=${list.userId}"
			...
			<td colspan="5"><c:forEach var="pageId"
					...
					<c:choose>
						<c:when test="${pageId!=pager.currentPage}">
							<a href="/cms/showAddActAction?pageId=${pageId}">${pageId}</a>
						</c:when>
						<c:otherwise>
