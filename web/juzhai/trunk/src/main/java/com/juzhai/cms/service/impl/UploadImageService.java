...
public class UploadImageService implements IUploadImageService {
	...
	@Override
	public String uploadEditorTempImg(long id, String fileName,
			MultipartFile imgFile) throws UploadImageException {
		Date date=new Date();
		SimpleDateFormat sdf=new SimpleDateFormat("yyyy-mm-dd");
		return uploadTempImage(id, uploadTempImageHome+sdf.format(date)+File.separator, fileName, imgFile);

	}

	@Override
	public String uploadActTempImg(long id, String fileName,
			MultipartFile imgFile) throws UploadImageException {
		Date date=new Date();
		SimpleDateFormat sdf=new SimpleDateFormat("yyyy-mm-dd");
		return uploadTempImage(id, uploadTempImageHome+sdf.format(date)+File.separator, fileName, imgFile);
	...
	private String uploadTempImage(long id, String path, String fileName,
		...
		if (code == 1) {
			try {
				...
				FileUtil.writeStreamToFile(directoryPath, fileName,
						imgFile.getInputStream());
				return ImageUtil.generateHierarchyImageWebPath(id,
