	...
	<table border="0" cellspacing="4">
		...
		<c:forEach var="cmsActMagerView" items="${cmsActMagerViews}"
			...
			<tr>
				...
				<td>省份：${cmsActMagerView.proName}<br/> 城市：${cmsActMagerView.cityName}<br/> 详细地址：${cmsActMagerView.address}<br/></td>
				<td>年龄：${cmsActMagerView.age}<br/>  性别：${cmsActMagerView.gender}<br/>  群体：${cmsActMagerView.status}</td>
			...
			<td colspan="12"><c:forEach var="pageId"
					...
					<c:choose>
						<c:when test="${pageId!=pager.currentPage}">
							<a
								onclick="actManagerPage('${pageId}','${bDate}','${eDate}','${name}')" href="#">${pageId}</a>
						</c:when>
						<c:otherwise>
