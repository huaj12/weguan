 ...
 * 平台宫格列表显示工具。
 * <p>
 * WeiboGridView对“android.support”包有依赖，因此请注意检查您项目中是 否已经集成了相应的jar包
 */
public class WeiboGridView extends LinearLayout implements Runnable,
	...
	private void init(final Context context) {
		...
		int dp_10 = UIUtil.dip2px(context, 10);
	...
	public void run() {
		...
		// 计算pager的页数和高度
		int pageSize = weiboList.length > PAGE_SIZE ? PAGE_SIZE
				: weiboList.length;
		...
		int dp_5 = UIUtil.dip2px(context, 5);
		Resources res = getResources();
		grayPoint = BitmapFactory.decodeResource(res, R.drawable.gray_point);
		whitePoint = BitmapFactory.decodeResource(res, R.drawable.white_point);
	...
	public boolean handleMessage(Message msg) {
		...
		case 1: { // 成功
			showNotification(2000,
					getResources().getString(R.string.share_completed));
		}
			break;
		case 2: { // 失败
			if (msg.obj instanceof WechatClientNotExistException) {
				showNotification(
						...
						getResources().getString(
								R.string.wechat_client_not_install));
			} else if (msg.obj instanceof WechatTimelineNotSupportedException) {
				showNotification(
						2000,
						getResources().getString(
								R.string.wechat_timeline_not_support));
			} else {
				showNotification(2000,
		...
		case 3: { // 取消
			showNotification(2000,
					getResources().getString(R.string.share_canceled));
		}
			break;
		}
	...
	private static class GridView extends LinearLayout {
		...
		private void init() {
			int dp_10 = UIUtil.dip2px(getContext(), 10);
			int scrW = getResources().getDisplayMetrics().widthPixels;
		...
		private String getName(AbstractWeibo weibo) {
			...
			return getContext().getResources().getString(
					getContext().getResources().getIdentifier(
							weibo.getName().toLowerCase(), "string",
							getContext().getPackageName()));
		}
